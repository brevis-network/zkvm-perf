...
use crate::{
    utils::{
        gas_amount, get_elf, hash_bytes_per_second, hashes_per_second, rand_ecdsa_signature,
        rand_eddsa_signature, time_operation,
    },
    HashFnId, ProgramId,
};
#[cfg(feature = "risc0")]
...
use methods::{FIBONACCI_ELF, FIBONACCI_ID, RSP_ELF, RSP_ID, TENDERMINT_ELF, TENDERMINT_ID};
...
impl Risc0Evaluator {
    ...
    pub fn eval(args: &EvalArgs) -> PerformanceReport {
        ...
        let (elf, image_id) = match args.program {
            ProgramId::Fibonacci300kn => (FIBONACCI_ELF, FIBONACCI_ID),
            ProgramId::Tendermint => (TENDERMINT_ELF, TENDERMINT_ID),
            ProgramId::Rsp17106222 => (RSP_ELF, RSP_ID),
            ProgramId::Rsp20528709 => (RSP_ELF, RSP_ID),
            _ => {
                unreachable!()
            }
        };
        ...
        match args.program {
            ...
            ProgramId::Groth16ProofVerify => {
                ...
                const VERIFICATION_KEY: &str =
                    include_str!("../../fixtures/risc0/verification_key.json");

                // Verification_key, proof and public witness generated by SnarkJS using Groth16
                // over BN254 (https://docs.circom.io/getting-started/proving-circuits/)
                let proof_json: ProofJson = serde_json::from_str(PROOF).unwrap();
                let public_inputs_json =
                    PublicInputsJson { values: serde_json::from_str(PUBLIC_INPUTS).unwrap() };

                let verifying_key_json: VerifyingKeyJson =
